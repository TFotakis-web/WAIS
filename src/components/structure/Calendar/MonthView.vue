			...
			<div class="mdb-month-view">
				<div class="d-flex justify-content-between mb-3">
					<mdb-btn-group size="sm">
						<mdb-btn size="sm" outline="info" icon="chevron-left" darkWaves @click="changeMonth('prev')"></mdb-btn>
						<mdb-btn size="sm" outline="info" icon="chevron-right" darkWaves @click="changeMonth('next')"></mdb-btn>
						<mdb-btn class="px-3" outline="info" darkWaves @click="changeMonth('today')">{{ todayText }}</mdb-btn>
					</mdb-btn-group>
					<mdb-btn-group>
						<mdb-btn color="info" darkWaves @click="changeView('month')">{{ $t('components.calendar.month') }}</mdb-btn>
						<mdb-btn outline="info" darkWaves @click="changeView('week')">{{ $t('components.calendar.week') }}</mdb-btn>
						<mdb-btn outline="info" darkWaves @click="changeView('list')">{{ $t('components.calendar.list') }}</mdb-btn>
					</mdb-btn-group>
				</div>
				<table>
					<thead>
					...
					<tr v-for="(row, rowIndex) in 6" :key="'tr' + row.toString()">
						<drop
							...
							@click.stop="addEvent($event, day.date)"
						>
