			...
			<div class="mdb-week-view">
				<div class="d-flex justify-content-between mb-3">
					<mdb-btn-group size="sm">
						<mdb-btn size="sm" outline="info" icon="chevron-left" darkWaves @click.native="changeWeek('prev')"></mdb-btn>
					...
					<mdb-btn-group>
						...
						<mdb-btn color="info" darkWaves @click.native="changeView('week')">{{ $t('Week') }}</mdb-btn>
				...
				<table>
					<thead>
						<tr>
							<th></th>
							<th v-for="(weekDay, index) in dayNames" :key="index" :class="{ 'light-blue lighten-5': isToday(index) }">
								{{ dayNames[(firstDay.getDay() + index) % 7] }}
								{{ new Date(firstDay.getFullYear(), firstDay.getMonth(), firstDay.getDate() + index) | moment('DD/MM') }}
							</th>
						</tr>
						<tr>
							<th class="text-center font-weight-bold">{{ $t('All day') }}</th>
							<drop
								tag="td"
								v-for="(weekDay, dayIndex) in dayNames"
								:key="dayIndex"
					...
					<tbody>
						<tr v-for="(hour, hourIndex) in 24" :key="hourIndex">
							<th>{{ hour }}:00</th>
							<drop
								tag="td"
