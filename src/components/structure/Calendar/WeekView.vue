				...
				<mdb-col>
					<div class="d-flex justify-content-between mb-3">
						<mdb-btn-group size="sm">
							<mdb-btn size="sm" outline="info" icon="chevron-left" darkWaves @click="changeWeek('prev')" class="ml-0"></mdb-btn>
						...
						<mdb-btn-group>
							...
							<mdb-btn color="info" darkWaves @click="changeView('week')">{{ $t('components.calendar.week') }}</mdb-btn>
							<mdb-btn outline="info" darkWaves @click="changeView('list')" class="mr-0">{{ $t('components.calendar.list') }}</mdb-btn>
						</mdb-btn-group>
					</div>
				</mdb-col>
			</mdb-row>
											...
											<div v-for="(event, eventIndex) in events" :key="eventIndex">
												<mdb-tooltip v-if="showTooltip(event, dayIndex)" material trigger="hover" :options="{ placement: 'top' }">
													...
													<drag
														...
														@click.stop="editEvent(eventIndex)"
													>{{ event.title }}
													</drag>
												</mdb-tooltip>
											</div>
							...
							<mdb-row v-for="(hour, hourIndex) in 24" :key="hourIndex">
								...
								<mdb-col v-for="(day, dayIndex) in 7" :key="dayIndex" class="px-0">
									<div class="mdb-week-view-cell">
										<drop
											tag="div"
											@click.stop="addEvent($event, dayIndex, hourIndex)"
											@drop="handleDrop($event, dayIndex, hourIndex)"
											:class="getClassesDropTBody(dayIndex)"
											class="pb-4 h-100 w-100"
											@mouseover="tdHover(dayIndex, hourIndex)"
										>
											<div v-for="(event, eventIndex) in events" :key="eventIndex">
												<mdb-tooltip
